// app/(dashboard)/dashboard/concerts/[id]/page.tsx
      ...
      <div className="grid gap-6 md:grid-cols-2">
          ...
          <CardContent className="space-y-4">
            {concert.conIntroduction && (
              <div>
                ...
                <p className="text-sm whitespace-pre-wrap">{concert.conIntroduction}</p>
              </div>
            )}

            {concert.ticketPurchaseMethod && (
              <div>
                <p className="text-sm font-medium text-muted-foreground mb-2">購票方式</p>
                <p className="text-sm whitespace-pre-wrap">{concert.ticketPurchaseMethod}</p>
              </div>
            )}

            {concert.precautions && (
              <div>
                <p className="text-sm font-medium text-muted-foreground mb-2">注意事項</p>
                <p className="text-sm whitespace-pre-wrap">{concert.precautions}</p>
              </div>
            )}

            {concert.refundPolicy && (
              <div>
                <p className="text-sm font-medium text-muted-foreground mb-2">退票政策</p>
                <p className="text-sm whitespace-pre-wrap">{concert.refundPolicy}</p>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        {concert.reviewNote && (
          <Card className="md:col-span-2">
            ...
            <CardContent>
              <p className="text-sm whitespace-pre-wrap">{concert.reviewNote}</p>
            </CardContent>
          </Card>
        )}
          ...
          <CardContent>
            <div className="grid gap-2 text-sm">
              <div className="flex justify-between">
                ...
                <span>{format(new Date(concert.createdAt), "yyyy/MM/dd HH:mm:ss", { locale: zhTW })}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">最後更新</span>
                <span>{format(new Date(concert.updatedAt), "yyyy/MM/dd HH:mm:ss", { locale: zhTW })}</span>
              </div>
              {concert.cancelledAt && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">取消時間</span>
                  <span>{format(new Date(concert.cancelledAt), "yyyy/MM/dd HH:mm:ss", { locale: zhTW })}</span>
                </div>
              )}
            </div>
          </CardContent>
